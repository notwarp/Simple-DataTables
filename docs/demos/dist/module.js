...
const isJson = (str) => {
    ...
    return !(null === t || (!Array.isArray(t) && !isObject$1(t))) && t;
};
/**
...
class DataTable {
    ...
    _remoteFetch(sort = undefined) {
        ...
        switch (remote.method) {
            ...
            case "POST":
                return axios$1.post(remote.url, remoteBodyDatas, {
                    headers: {
    ...
    _renderPage(lastRowCursor = false) {
        ...
        if (this.totalPages) {
            ...
            f = f + 1;
            items = this._searchQueries.length ? this._searchData.length : (!this.hasRemote) ? this.data.data.length : this.options.remote.resultsData.totalRecords;
        }
    ...
    _bindEvents() {
        ...
        // Pager(s) / sorting
        this.wrapperDOM.addEventListener("click", async (event) => {
            const target = event.target;
...
export { DataTable, convertCSV, convertJSON, createElement, exportCSV, exportJSON, exportSQL, exportTXT, isJson, isObject$1 as isObject, makeEditable };
